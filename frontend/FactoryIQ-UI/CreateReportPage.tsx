        ...
        if (data.ok) {
          if (tplReportType === "custom") {
            ...
            (data.data || []).forEach((row: any) => {
             ...
             const rawVal = row["Прирост_кг"];

              
              groupedRows[key][row.TagName] =
  rawVal !== undefined && rawVal !== null ? Number(rawVal) : "-";
      ...
      .then((data) => {
        if (data.ok) {
          ...
          (data.data || []).forEach((row: any) => {
            ...
            groupedRows[key][row.TagName] = isNaN(parsed) ? "-" : parsed;
  ...
  const exportToExcel = () => {
    ...
    if (reportType === "balance") {
      ...
      selectedTags.forEach((t) => {
        ...
        const tagDisplayName = (t.tag.description || t.tag.browse_name || t.tag.name)
          ?.replace(/,.*$/, "")
          ?.trim();

        totals[tagDisplayName] = dailyRows.reduce((sum, row) => {
          const value = row[tagDisplayName];
          if (value == null || value === "" || value === "-") return sum;
                ...
                <table className={styles.reportWideTable}>
                  <thead>
                    ...
                    {selectedTags.map((t, i) => {
                      const cleanName = (t.tag.description || t.tag.browse_name || t.tag.name)
                          ?.replace(/,.*$/, "")     // удаляем всё после запятой
                          ?.trim();

                      return <th key={i}>{cleanName}</th>;
                  })}
                  ...
                  <tbody>
                    {reportTableRows.map((row, idx) => (
                      <tr key={idx}>
                        ...
                        <td>{row["Смена"] ?? "-"}</td>
                            {selectedTags.map((t, i) => {
                                const tagName = (t.tag.description || "")
                                  .replace(/,.*$/, "")       // убираем всё после запятой
                                  .trim();

                                const val = row[tagName];
                                return <td key={i}>{val == null ? "-" : val}</td>;
                              })}
                              </tr>
                    ))}
                    ...
                    {showDailySum && (
                      <tr style={{ fontWeight: "bold", background: "#e3fbfa" }}>
                        ...
                        {selectedTags.map((t, i) => {
                          ...
                          return (
                            <td key={i}>
                              {totals[tagDisplayName]?.toLocaleString("ru-RU", {
